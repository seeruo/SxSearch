;(function (window, document) {
    var SxSearch = function (target, options) {
        // 检查是否是本对象
        if(!(this instanceof sxsearch)){
            return new sxsearch(target, options);
        }
    ...
    SxSearch.prototype = {
        ...
        renderDialog() {
            ...
            dialog.classList.add("sxsearch-dialog");

            var dlg_html = '<div class="sxsearch-dialog-mask"></div>';
            dlg_html += '<div class="sxsearch-dialog-module">';
            dlg_html += '<div class="sxsearch-dialog-close">X</div>';
            dlg_html += '<div class="sxsearch-dialog-body">';
            dlg_html += '<div class="sxsearch-dialog-input"><input class="sxsearch-input" placeholder="搜索"></div>';
            dlg_html += '<div class="sxsearch-dialog-scroll"><div class="sxsearch-dialog-main"></div></div>';
            dlg_html += '</div>';
            dlg_html += '</div>';
        ...
        bindDialogEvent: function () {
            ...
            // 关闭对话框
            var closeBtnDom = this.target.querySelector('.sxsearch-dialog-close');
            ...
            // 关闭对话框
            var searchBtnDom = this.target.querySelector('.sxsearch-dialog .sxsearch-input');
            searchBtnDom.addEventListener("input",function(){
                _this.handleSearch(this.value);
            }, false);
        },
        closeDialog(){
            var dialog = this.target.querySelector('.sxsearch-dialog');
            dialog.classList.remove("sxsearch-dialog-show");
        },
        showDialog(){
            var dialog = this.target.querySelector('.sxsearch-dialog');
            dialog.classList.add("sxsearch-dialog-show");
        },
        handleSearch(input){
            var result = [];
            if (input !== ''){
        ...
        renderList(input){
            ...
            var showArea = this.target.querySelector('.sxsearch-dialog-main');
